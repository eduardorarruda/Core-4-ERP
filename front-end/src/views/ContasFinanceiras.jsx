...
export default function ContasFinanceiras() {
  ...
  async function criar(e) {
    e.preventDefault();
    if (valorLiquido <= 0) {
      setToast({ message: 'Valor líquido deve ser maior que zero.', type: 'error' });
      return;
    }
    setSalvando(true);
    try {
  ...
  async function estornar(id) {
    if (!confirm('Cancelar a quitação desta conta? O saldo da conta corrente será estornado.')) return;
    try {
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={criar} className="bg-surface-low rounded-2xl p-6 border border-white/5 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="space-y-1">
            <label className={labelCls}>{form.tipo === 'PAGAR' ? 'Fornecedor' : 'Cliente'}</label>
          ...
          <div className="space-y-1">
            <label className={labelCls}>Valor Líquido (R$)</label>
