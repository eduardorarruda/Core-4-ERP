...
export default function ConciliacaoCartaoRelatorio() {
  ...
  function exportarPDF() {
    ...
    try {
      ...
      exportarRelatorioConciliacaoPdf({
        titulo: `Relatório de Conciliação de Cartão #${rel.id}`,
        subtitulo: rel.cartaoCreditoNome,
        cabecalho: ['Data', 'Descrição OFX', 'Valor OFX', 'Lançamento', 'Valor Lanç.', 'Status', 'Score'],
        linhas,
        nomeArquivo: `ConciliacaoCartao_${rel.id}_${rel.dataInicioOfx ?? ''}.pdf`,
      });
  ...
  if (erro || !rel) {
    return (
      <div className="space-y-6">
        <PageHeader title="Relatório de Conciliação de Cartão" subtitle="Conciliação de cartão de crédito" />
        <EmptyState
          icon={AlertTriangle}
          ...
          action={{ label: 'Voltar ao histórico', onClick: () => navigate('/cartoes/conciliacao/historico') }}
        />
      </div>
    );
  }
  ...
  const STATS = [
    ...
    { label: 'Confirmados', value: rel.totalConciliados, icon: CheckCircle2, variant: 'primary' },
    { label: 'Ignorados', value: rel.totalIgnorados, icon: MinusCircle, variant: 'muted' },
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-3">
            <Button variant="secondary" leftIcon={<ArrowLeft className="w-4 h-4" />} onClick={() => navigate('/cartoes/conciliacao/historico')}>
              Voltar
            </Button>
