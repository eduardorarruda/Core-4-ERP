...
| IA | Spring AI + OpenAI | gpt-4o-mini |
...
O assistente usa **Spring AI com OpenAI (gpt-4o-mini)** e um padrão de **tools** (function calling).
...
- `LancamentoCartao` possui campo `tipo: TipoLancamentoCartao (SAIDA | ENTRADA)` — V36 aplicada.
  - Toda query de soma de lançamentos deve calcular: `SUM(SAIDA) − SUM(ENTRADA)` = total líquido.
  - `fecharFatura` usa o total líquido; fatura com total ≤ 0 não pode ser fechada.
- Regra de alocação de fatura: `FaturaHelper.calcularFatura(dataCompra, diaFechamento)` — se `dia >= diaFechamento`, vai para o mês seguinte. Usada em `CartaoCreditoService` e `ConciliacaoCartaoService`.
...
- **Filtro temporal dinâmico:** `GET /api/dashboard` e `GET /api/cartoes/dashboard/resumo` aceitam parâmetros opcionais `mesInicio`, `anoInicio`, `mesFim`, `anoFim`. Quando omitidos, comportamento padrão é preservado.
- **Dashboard BI de Cartões:** `GET /api/cartoes/dashboard/bi` com 7 painéis — evolução mensal, distribuição de limite, gastos por parceiro, próximas faturas, projeção do mês, impacto de parcelamentos, assinaturas vs. avulsos. Retorna também `alertasFechamento` (§8). Ver `CartaoDashboardBIResponseDto`.
...
## 13. Features em Radar / Escopo Planejado

> Todas as funcionalidades detalhadas em `Melhorias_Cartao.md` foram implementadas em 2026-06.

| Feature | Arquivo de ref. | Status |
|---|---|---|
| Expansão de ícones de categorias (20→52) | `Melhorias_Cartao.md §1` | **Implementado** |
| Busca de categoria/parceiro em lançamentos de cartão | `Melhorias_Cartao.md §2` | **Implementado** |
| Filtro temporal dinâmico (Dashboard + Dashboard Cartão) | `Melhorias_Cartao.md §3` | **Implementado** |
| ModalCriacaoRapida na Conciliação (Parceiro + Categoria) | `Melhorias_Cartao.md §4` | **Implementado** |
| Campo `tipo` em lançamento de cartão (ENTRADA/SAIDA) + V36 | `Melhorias_Cartao.md §5` | **Implementado** |
| `FaturaHelper` + fix conciliação cartão (data fechamento) | `Melhorias_Cartao.md §6` | **Implementado** |
| Endpoint `/api/cartoes/dashboard/bi` (7 painéis BI) | `Melhorias_Cartao.md §7` | **Implementado** |
| Alerta visual de fechamento de fatura iminente (≤ 7 dias) | `Melhorias_Cartao.md §8` | **Implementado** |
| Fix `/api/auditoria` 500 — native query com CAST (PSQLException nullable param) | `implementation_plan.md` | **Implementado** |
| `AuditoriaController` — `@PageableDefault` no lugar de `PageRequest.of` manual | `implementation_plan.md` | **Implementado** |
| Fix `/api/cartoes/dashboard/bi` — validação parâmetros parciais mes/ano | `implementation_plan.md` | **Implementado** |
| `cpfCnpj` obrigatório (`@NotBlank`) em `ParceiroRequestDto` | `implementation_plan.md` | **Implementado** |
| Dashboard Cartões — estado inicial 30 dias + botões rápidos + `DateRangeModal` | `implementation_plan.md` | **Implementado** |
| `IconDropdown.jsx` — seleção de ícone com grid + busca + agrupamento | `implementation_plan.md` | **Implementado** |
| `ModalCriacaoRapida` — parceiro agora exige `cpfCnpj`/`tipo`; categoria usa `IconDropdown` | `implementation_plan.md` | **Implementado** |
| `FormularioNovaContaInline` — botões `+` Categoria e Parceiro (conciliação bancária) | `implementation_plan.md` | **Implementado** |
| `Parceiros.jsx` — badge Receita Federal + highlight campos autopreenchidos | `implementation_plan.md` | **Implementado** |
...
### JPQL nullable params — PostgreSQL não infere tipo do parâmetro

`(:param IS NULL OR a.campo = :param)` em JPQL com parâmetro nullable gera dois bind params `?` para o mesmo valor. O PostgreSQL não consegue inferir o tipo do primeiro `?` sem contexto → `PSQLException: could not determine data type of parameter $N`.

**Solução:** trocar para native query com `CAST(:param AS TIPO)`:
```sql
...
Parâmetros de enum em native query devem ser passados como `String` (`acao.name()`) — o repositório recebe `String` em vez do enum Java.
...
- Nunca exibir ao usuário mensagens técnicas — códigos de permissão, nomes de exceção, nomes de tabela/constraint, stack traces. Toda notificação visível ao usuário deve ser clara e compreensível por qualquer pessoa sem conhecimento técnico (ver seção 8 → "Mensagens ao Usuário").

---

## 16. Restrições de Banco de Dados
...
## 18. Fluxo de Estados dos Documentos
...
## 19. Integridade Referencial e Regras Cruzadas
...
## 20. Regras Legais e de Formatação (Contexto Brasileiro)
...
## 21. Integrações Externas — Autenticação e HTTP Client

O sistema possui três integrações externas ativas:

| Integração | Mecanismo | Credencial |
|---|---|---|
| OpenAI (IA) | API Key no header | `OPENAI_API_KEY` no `.env` |
...
### HTTP Client
- Integração com OpenAI é gerenciada pelo **Spring AI** (`spring-ai-openai-spring-boot-starter`) — não fazer chamadas HTTP diretas ao endpoint.
...
- Chamadas à OpenAI: Spring AI gerencia retry interno (spring.ai.retry.max-attempts=2). Se o modelo retornar erro, propagar como `ServiceUnavailableException` — nunca deixar virar 500 genérico.
...
## 22. Contratos de API — Padrões de Resposta e DTOs
...
## 23. Agentes de IA — Diretrizes Específicas

### Configuração do modelo
- Modelo: **OpenAI gpt-4o-mini** via Spring AI (configurável por `OPENAI_MODEL`).
- Configuração em `application.properties` referenciando `OPENAI_API_KEY` do `.env`.
...
- Se a OpenAI estiver indisponível: retornar HTTP 503 com `Retry-After` — não tentar fallback para outro modelo sem decisão explícita de arquitetura.

---

## 24. Checklist de PR para Integrações Novas
